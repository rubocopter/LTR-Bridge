...
Status: **local D3D11 x86 -> D3D12 x64 multiframe transport is implemented and host-tested across repeated frames, live resource replacement, process/device loss, bounded backpressure, stereo/history isolation and renderer-transfer variants; one real Call of Juarez frame crosses x86 -> x64, and the exact two-slot BGRA8 client is host-tested offline and connected to the observer in compile-tested source, while live multiframe and real resource-generation validation remain pending**.
...
Microsoft's broader interoperability documentation also says unsynchronized sharing is supported by D3D9Ex while D3D9c and older runtimes do not support shared surfaces. The local Win32 probe agrees with that narrower reading on the current host: classic D3D9 returns `D3DERR_INVALIDCALL` for every tested shared-texture creation, and also fails to open R10/RGBA16F/BGRA8-control handles created by a separate D3D9Ex device. D3D9Ex itself successfully exports R10G10B10A2 and RGBA16F resources that D3D11 opens as `SRV|RTV`. These results remain host/driver-scoped evidence rather than a universal Windows rule.
...
2. D3D9-facing game -> D3D9Ex-compatible root/device substitution -> dedicated relay -> private D3D11; Call of Juarez host-tests this route through the first real-frame D3D11 boundary;
...
The clean Steam Call of Juarez build proves the missing join as a one-shot experiment. Its intercepted D3D9-facing device is backed by D3D9Ex, the real `2560x1440` A8R8G8B8 frame enters a D3D9Ex shared relay, private x86 D3D11 opens it, then copies it to an NT-shared BGRA8 resource. A separate x64 D3D12 helper opens that NT handle and validates five points with zero mismatches. This diagnostic path waits synchronously, launches a child helper for one frame and uses readback; its purpose is to prove that no additional resource-format or bitness boundary blocks the real game.

The game-topology transport mechanics have since been isolated from the renderer in `src/d3d9_real_bridge_probe/`. The first D3D11-owned persistent-slot reproduction advanced shared fence values but failed actual content validation with 12/12 zero-valued D3D12 samples. The passing design therefore reuses the bridge's established ownership direction: x64 D3D12 creates two shared BGRA8 slots and the `done` fence, duplicates those handles into x86, x86 D3D11 opens them and owns `ready`, and both processes retain explicit adapter/protocol/resource contracts and child-liveness checks. Its x86 client is shared by the synthetic producer and real observer. After that extraction, five `64x64` and three `2560x1440` 12-frame repetitions completed with zero mismatches and forced depth-2 backpressure. A later no-game optimization ties two D3D9/D3D11 source relays to those same ring slots: because `done` is downstream of D3D12's wait on `ready`, and `ready` is downstream of the D3D11 source copy, slot admission also proves that the corresponding source relay is safe to overwrite. This removes the observer's separate per-frame D3D11 event-query wait. The synthetic producer was tightened to wait for slot admission before rewriting its matching source texture; after that change 3/3 `64x64` and 2/2 `2560x1440` forced-backpressure runs again completed 12/12 with zero mismatches. The observer integration remains compile-tested and adds D3D9 completion synchronization, capture skipping under backpressure, reset generation recreation and fail-open teardown. This closes the transport-mechanics and source-code join gates; live real-game multiframe and lifecycle evidence remain pending.
