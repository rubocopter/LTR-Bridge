...
Status: **local D3D11 x86 -> D3D12 x64 multiframe prototype implemented and host-tested; repeated frames, true mid-run resource replacement, controlled process/device loss, bounded ring-depth backpressure, two-eye isolation, synthetic per-eye GPU-history isolation/reset semantics, timestamp/copy accounting, a small format/interoperability matrix and negative-path diagnostics are covered; broader failure timing and OpenXR/headset execution remain pending**.
...
### Renderer-local -> shared-resource copy probe
...
This closes only the minimal copy-accounting question for small same-format, single-sample synthetic textures on the current host. It does not establish the cost of a real game's render target, MSAA resolve, format conversion, scaling, resource hazards, higher resolutions or copying two full-resolution VR eyes. These numbers are not `performance-validated`.
...
The deterministic multiframe transport, one real mid-run resource replacement, synthetic renderer-to-shared copy accounting, dynamic-control rejection, a small format/interoperability matrix, abrupt client termination, abrupt host-loss detection at startup and during depth-1 slot reuse, controlled D3D12 device removal, bounded ring-depth backpressure, controlled two-eye transport isolation and synthetic per-eye GPU-history isolation/reset semantics are now covered. Broader format/sample-count coverage, mid-dispatch/device-loss timing, real-renderer/high-resolution copy behavior, real reconstruction-backend histories, OpenXR presentation and headset execution remain experiment-pending.
