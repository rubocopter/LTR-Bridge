...
**Implemented/host-tested architectural implication:** on this host the native D3D9Ex route reaches the existing modern transport end to end: bound local D3D9Ex R10 render target -> clear/restore -> `StretchRect` -> shared R10 relay -> private x86 D3D11 fullscreen conversion -> host-created shared RGBA8 transport -> x64 D3D12 consumer. Five dedicated 24-frame repetitions pass at `64x64 -> 96x72` and five more at `1920x1080 -> 2560x1440`; every run survives one `ResetEx`/generation transition with zero mismatches. R10 validation allows `rgb8_plus_minus_1_lsb` to account for D3D9 render-target quantization. At high resolution, the D3D9Ex bind/clear/restore/relay/event interval averaged about `0.3637 ms` across run means and the D3D11 relay-to-RGBA8 shader interval about `19.5638 us`. These controlled single-host synthetic measurements are not performance validation. True classic D3D9 still cannot use that sharing boundary directly on this host.

**Implemented/host-tested controlled-scene implication:** a separate x86 D3D9Ex scene probe owns default-pool geometry, R10 color and D24S8 depth resources and uses fixed-function world/view/projection state plus two overlapping depth-tested draw calls. The capture occurs while the engine color/depth resources remain bound; the probe confirms the current render target, current depth surface and per-frame world transform before `StretchRect`, then validates depth occlusion after D3D11 readback. Five `640x360 -> 1280x720` reset/recreation runs pass with zero mismatches. The same scene reaches the existing x64 bridge in a dedicated mode, with five more 24-frame zero-mismatch runs.

**Implemented/host-tested external-boundary implication:** the scene now also runs in a separate target executable with capture performed by a separately built interceptor DLL. The target only loads and installs the interceptor; it does not hand renderer resources or per-frame state to it. The interceptor hooks `Direct3DCreate9Ex`, `CreateDeviceEx`, `EndScene` and `ResetEx`, discovers the bound R10 color target, D24S8 depth and world transform after `EndScene`, copies to its own D3D9Ex shared relay, validates through private D3D11 and verifies that the observed target/depth/transform are unchanged after capture. Five 12-frame `640x360 -> 1280x720` runs pass through one intercepted reset with zero mismatches and clean shutdown. This moves the controlled result behind a real process-local API interception boundary while remaining synthetic and cooperatively loaded; it is not evidence from an arbitrary injected game process.

**Implemented/host-tested real-game implication:** the clean Steam Call of Juarez build resolves `Direct3DCreate9` dynamically and normally creates a true classic-D3D9 device. A temporary dependency forwarder/bootstrap can intercept that call and, in an opt-in experiment, return a `Direct3DCreate9Ex` root through the base `IDirect3D9` interface. The game's unchanged `CreateDevice` call then returns an Ex-capable device and continues to the same `2560x1440` fullscreen `Present` boundary. Before one real `Present`, the probe copies the A8R8G8B8 game target into a shared same-format texture, waits on a D3D9 event query, opens the handle with D3D11, and compares validation readback from both APIs. Five sample pixels match exactly. The observer also sees D24X8 depth, transforms and one swapchain.

**Implemented/host-tested cross-bitness implication:** the exact real-game relay now crosses into a separate x64 D3D12 process. Private x86 D3D11 copies the opened D3D9 relay into a BGRA8 resource created with `D3D11_RESOURCE_MISC_SHARED_NTHANDLE | D3D11_RESOURCE_MISC_SHARED_KEYEDMUTEX`, waits on a D3D11 event query and exports an inheritable NT handle through `IDXGIResource1::CreateSharedHandle`. A minimal x64 D3D12 helper opens the resource, copies it into a readback buffer and compares the same five sample points; all match. The first attempt used only `SHARED_NTHANDLE` and `CreateTexture2D` returned `E_INVALIDARG`; Microsoft's `IDXGIResource1::CreateSharedHandle` documentation explicitly requires the NT-handle resource to have both `SHARED_NTHANDLE` and `SHARED_KEYEDMUTEX`, and the corrected combination passes on this host. This is a synchronous one-frame proof of the real resource boundary, not the final fence/ring transport loop.
...
The local probes have also moved several earlier hypotheses into **host-tested** territory: the D3D11 x64 temporal harness, GPU-resident D3D11 x86 -> D3D12 x64 transport, bounded queueing and failure handling, synthetic per-eye isolation/history, XeSS 3.0.2 Native AA execution, OpenXR runtime bootstrap, the current-host D3D9Ex/D3D10.1 -> private D3D11 -> modern-host relays, and one real Call of Juarez D3D9-facing color frame crossing through D3D11 x86 into D3D12 x64. The real-game crossing is still synchronous and one-shot; these results do not establish sustained real-game reconstruction, multi-host, performance, OpenXR-presentation or headset support.
...
1. Live-test the already connected Call of Juarez D3D9Ex -> private-D3D11 -> x64-owned two-slot ready/done path, then prove reset/resource-generation/fail-open lifecycle and record scene-color/depth/pass/HUD and temporal-input behavior before deciding whether the dgVoodoo D3D12-addon route offers a better compatibility/provenance tradeoff.
