...
| D3D9Ex -> D3D11 relay -> x86/x64 D3D12 host | host-tested | current-host R10 relay works end to end | host-tested | unknown | unknown | unknown | Local D3D9Ex R10 render target -> `StretchRect` -> shared R10 relay -> private D3D11 fullscreen conversion -> existing shared RGBA8 transport -> x64 D3D12; one `ResetEx`/`64x64 -> 96x72` transition, 24 frames, zero mismatches, five repetitions. RGBA16F is also proven in the independent relay probe; documented RGBA8 fails here, while BGRA8 remains a driver-specific control. |
