...
The current work expands the host-tested D3D11 x86 -> D3D12 x64 bridge from one round trip to a 24-frame/two-generation deterministic probe with separate unidirectional fences, D3D12 timestamps, explicit copy accounting and negative-path diagnostics. Generated build directories and the external SDK cache remain ignored artifacts and are not part of the repository state.
...
38. **Implemented/host-tested:** the D3D11 x86 -> D3D12 x64 bridge now runs 24 deterministic frames: 12 at `64x64`, then 12 at `96x72`, with `0` pixel mismatches. The two x64-host-created resource generations are opened by D3D11 and transformed in-place by D3D12; CPU readback is validation-only. The generations are pre-created before process launch, so this is a generation/size-transition test, not dynamic handle replacement.
...
40. **Host-tested:** five additional 24-frame positive runs all passed with zero mismatches. Their per-run mean D3D12 compute intervals averaged `3.558 us` with a `3.499–3.669 us` range between run means. Their per-run mean x86 signal-to-validation-readback wall times averaged `1.5022 ms` with a `1.3372–1.6114 ms` range between run means; this includes synchronization, a staging GPU copy and CPU readback. The D3D12 transform records `0` transport copies; the test performs one validation copy per frame. These are short single-host/synthetic measurements, not `performance-validated`.
41. **Host-tested negative paths:** protocol version mismatch, invalid adapter LUID, resource-generation extent mismatch and a missing-host-response timeout all fail deterministically with explicit diagnostics. The host-stall path exits after `250 ms` rather than blocking indefinitely. Real process termination/device removal and client-loss cleanup remain pending.
42. **Experiment-pending:** dynamic bridge resource rebuild/handle replacement, actual host/client termination, device removal, unsupported-format matrix, backpressure/ring depth, renderer-to-shared-resource copy cost, stereo contamination testing, real skinned/cloth content, real particle systems, real-engine/general transparency/HUD policy, real-content optical-flow validation, direct renderer-resource transfer, longer/multi-host performance and total-VRAM measurement, additional backend mappings, SR, and stereo/OpenXR execution.
...
7. The bridge now covers repeated frames, a pre-created size-generation transition, timing/copy accounting and protocol/adapter/resource-contract/host-stall failures. Next resolve dynamic resource replacement and real process/device-loss cleanup, then add bounded ring depth/backpressure measurements. When stereo is added, use two independent streams, distinct histories/resources, deterministic readback and cross-eye contamination detection. OFXR Bridge informs the later OpenXR presentation/lifetime stage but is not a transport dependency.
