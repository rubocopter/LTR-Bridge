...
28. **Experiment-pending:** real skinned/cloth content, real particle systems, real-engine/general transparency and mitigation policy, HUD/highlights, real-content optical-flow validation, native-AA backend mapping, frame-time/VRAM measurement, SR, and stereo.
...
5. Continue controlled content coverage with a deterministic HUD/highlight case. Preserve the blended-transparency result as an unresolved mixed-layer requirement; do not choose a mitigation until backend mappings and real content justify one. Keep real skinned/cloth and particle systems explicitly pending before treating any provider as broadly representative.
