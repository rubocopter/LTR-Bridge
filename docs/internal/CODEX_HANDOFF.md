...
The current work expands the host-tested D3D11 x86 -> D3D12 x64 bridge to a 24-frame/two-generation deterministic probe with a real mid-run handle replacement, separate unidirectional fences, D3D12 timestamps, explicit copy accounting and controlled process/device-loss diagnostics. Generated build directories and the external SDK cache remain ignored artifacts and are not part of the repository state.
...
7. The bridge now covers repeated frames, true dynamic resource replacement, generation retirement, timing/copy accounting, protocol/adapter/resource-contract/host-stall/dynamic-control failures, abrupt client termination, controlled D3D12 device removal and abrupt host-loss detection. Next add bounded ring depth/backpressure measurements; broader failure timing can then be tested against that queueing model. When stereo is added, use two independent streams, distinct histories/resources, deterministic readback and cross-eye contamination detection. OFXR Bridge informs the later OpenXR presentation/lifetime stage but is not a transport dependency.
