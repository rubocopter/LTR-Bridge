...
5. **Implemented:** `--self-test` is registered in CTest and Windows CI. It renders reset/steady frames for static, camera translation, camera rotation, rigid-object motion and dedicated disocclusion; reads depth and `R32G32_FLOAT` motion back through staging textures; validates reset/history/frame identity and expected motion; and emits a text report plus diagnostic BMPs. Passing local/CI executions are **host-tested** evidence.
