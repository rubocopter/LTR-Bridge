...
The current work expands the host-tested D3D11 x86 -> D3D12 x64 bridge to a 24-frame/two-generation deterministic probe with a real mid-run handle replacement, separate unidirectional fences, D3D12 timestamps, explicit copy accounting, controlled process/device-loss diagnostics and a bounded depth-1/depth-2 backpressure experiment. Generated build directories and the external SDK cache remain ignored artifacts and are not part of the repository state.
...
7. The bridge now covers repeated frames, true dynamic resource replacement, generation retirement, timing/copy accounting, protocol/adapter/resource-contract/host-stall/dynamic-control failures, abrupt client termination, controlled D3D12 device removal, abrupt host-loss detection and bounded depth-1/depth-2 backpressure. The next transport probe should be controlled stereo: two independent streams with distinct resources/histories, deterministic readback and explicit cross-eye contamination detection. Broader failure timing can then be tested against the same bounded queue model. OFXR Bridge informs the later OpenXR presentation/lifetime stage but is not a transport dependency.
