...
The bridge now also has a synthetic renderer-copy mode: each frame is first written to a non-shared D3D11 texture and then copied on-GPU into the shared transport resource before the normal x64 path. This isolates and timestamps the missing renderer-to-shared copy step without treating the tiny controlled textures as representative game performance.
