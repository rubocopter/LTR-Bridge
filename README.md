...
The first legacy-API relay route is now host-tested end to end. A Win32 probe compares classic D3D9 and D3D9Ex sharing into D3D11 on the same adapter: classic D3D9 rejects the tested shared-texture creations, while D3D9Ex shares R10G10B10A2 and RGBA16F resources; a BGRA8 control also works on this driver even though it is outside the documented D3D9 -> D3D11 format list. The integrated D3D9Ex path now binds a local R10 render target, clears it through the active render-target state, restores the default target, copies with `StretchRect` into a shared R10 relay, waits for an event-query completion, opens that relay in a private x86 D3D11 device, converts it with a fullscreen shader into the existing shared RGBA8 transport, and reaches the x64 D3D12 host. Both the tiny `64x64 -> 96x72` profile and a new `1920x1080 -> 2560x1440` profile survive one `ResetEx`/resource recreation and validate 24 frames with zero mismatches across five dedicated repetitions. This is current-host evidence only, not general D3D9 support, real-game integration, VR validation, or performance validation.
