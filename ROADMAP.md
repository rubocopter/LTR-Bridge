...
Status: **multiframe transport, true mid-run resource replacement, controlled process/device-loss handling including host loss during depth-1 slot reuse, explicit synthetic renderer-to-shared GPU-copy accounting, bounded depth-1/depth-2 backpressure, controlled two-eye transport and synthetic independent per-eye GPU-history semantics implemented and host-tested; broader real-renderer copy cases, mid-dispatch failure timing and OpenXR/headset execution pending**.
