...
Status: **multiframe transport, true mid-run resource replacement, controlled process/device-loss handling including host loss during depth-1 slot reuse, bounded depth-1/depth-2 backpressure, controlled two-eye transport and synthetic independent per-eye GPU-history semantics implemented and host-tested with first timing/copy accounting and negative-path diagnostics; broader mid-dispatch failure timing and OpenXR/headset execution pending**.
