...
Status: **multiframe transport, true mid-run resource replacement, controlled process/device-loss handling, bounded depth-1/depth-2 backpressure and controlled two-eye transport implemented and host-tested with first timing/copy accounting and negative-path diagnostics; broader failure timing and OpenXR/headset execution pending**.
