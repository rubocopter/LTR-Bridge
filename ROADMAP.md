...
Current-host result: classic D3D9 returns `D3DERR_INVALIDCALL` for every tested `CreateTexture(..., pSharedHandle)` case. D3D9Ex successfully shares the documented R10/RGBA16F relay formats; documented RGBA8 fails here and BGRA8 remains a driver-specific control. The simpler relay path is stable through 1440p and `ResetEx`. The new controlled-scene probe renders overlapping red/green geometry through an engine-owned vertex buffer into an engine-owned R10 target with D24S8 depth and explicit fixed-function world/view/projection state. The capture observes that color target, depth surface and world transform while the resources remain bound, copies the bound target into the relay, and validates clear/color/depth-occlusion samples after D3D11 readback. Five standalone `640x360 -> 1280x720` runs pass with zero mismatches. The same scene is now a bridge mode; five 24-frame x86 -> x64 runs pass with one `ResetEx`/generation transition and zero mismatches. Across those bridge runs, the D3D9Ex scene-draw/capture CPU-wall mean averaged about `0.2881 ms` between run means and the D3D11 relay-to-RGBA8 shader about `5.0509 us`. These are controlled single-host measurements, not performance validation or real-game interception.
